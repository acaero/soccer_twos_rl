...
%PG
Ein ebenfalls verbreiteter Off-Policy Algorithmus im Bereich des \ac{RL} ist der \ac{PG} Algorithmus, welcher sowohl für kontinuierlichen als auch diskrete Aktionsräume geeignet ist. Bei diesem Verfahren wir direkt eine Policy-Funktion, in Form eines Neuronalen Netzes, erlernt, die angibt, welche Aktion in einem gegebenen Zustand ausgeführt werden soll.
...
Aufbauend auf den, bei der Implementierung der vorherigen Methoden gewonnen Erkentnissen, wurde im folgeden das \ac{MARL} Problem betrachtet. Da es sich hierbei um ein großes Themenfeld mit vielen, teils sehr komplexen Methoden handelt und bereits beim Training in einfacheren Single Agent Umgebung einige Herausforderungen auftraten wurden die Untersuchungen im \ac{MARL}-Bereich auf die Anwendung eines competitive self-plays mit dem \ac{PPO} Algorithms beschränkt.
